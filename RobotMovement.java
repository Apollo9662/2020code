...
public class RobotMovement extends LinearOpMode {
    ...
    public void followCurve(ArrayList<CurvePoint> allPoints) {
        ...
        this.speed = followMe.moveSpeed;
        this.turnSpeed = followMe.turnSpeed;
...
*/
        double x = p.x;
        double y = p.y;
        angle = Math.toDegrees(Math.atan2(y - robot.robotPosition.y,x- robot.robotPosition.x));
        error = getError(angle);
        steer = getSteer(error,0.001);

        robot.setDriveMotorsPower(-steer, Hardware.DRIVE_MOTOR_TYPES.LEFT);
        robot.setDriveMotorsPower(steer, Hardware.DRIVE_MOTOR_TYPES.RIGHT);
    }
