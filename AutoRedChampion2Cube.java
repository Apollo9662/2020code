...
@Autonomous(name = "Autonomous blue champion 2 Cube", group = "apollo")
@Disabled
public  class AutoRedChampion2Cube extends autonum{
    ...
    public void runOpMode(){
        ...
        pidDriveHighSpeed(0.6,-59,-92,normal,1);
        switch (vu){
            case right:
                pidDriveHighSpeed(0.6,-10,-92,normal,1);
                break;
            case center:
                pidDriveHighSpeed(0.6,-5,-92,normal,1);
                break;
        }
        sleep(50);
        ...
        Log.d(" robot x", String.valueOf((robot.rightCollector.getCurrentPosition() * (2048 * 4) / (2 * Math.PI))));
        Log.d(" robot x", String.valueOf((robot.rightCollector.getCurrentPosition() * (2048 * 4) / (2 * Math.PI))));
        pidDriveHighSpeed(0.8,15, -95,normal,1);
        pidDriveHighSpeed(0.8,28,-90,normal,1);
        Log.d(" robot x", String.valueOf((robot.rightCollector.getCurrentPosition() * (2048 * 4) / (2 * Math.PI))));
        Log.d(" robot x", String.valueOf((robot.rightCollector.getCurrentPosition() * (2048 * 4) / (2 * Math.PI))));
    ...
    public void catchCube2(){
        ...
        robot.leftCollector.setPower(-0.6);
        robot.rightCollector.setPower(0.6);

        int moved = (int) pidDriveByCube(0.2,-50,normal);
;
        pidDriveHighSpeed(0.6,moved + 5,-50,normal,1);
        gyroTurn(0.4,75);

        robot.leftCollector.setPower(1);
        robot.rightCollector.setPower(-1);

        pidDriveHighSpeed(0.8,50,88,normal,1);
