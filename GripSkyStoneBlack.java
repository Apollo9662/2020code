...
//		double[] hsvThresholdSaturation = {0.0, 201};
//		double[] hsvThresholdValue = {0.0, 53.12886054792529};
		...
		double blurRadius = 42.34234234234234;
		blur(blurInput, blurType, blurRadius, blurOutput);

		// Step Find_Blobs0:
		try {
			Mat findBlobsInput = blurOutput;
			findBlobs(findBlobsInput);
		}catch (Exception e){
			Log.d("error",e.getMessage());
		}
		Log.d("mat", String.valueOf(findBlobsOutput.size()));
	 ...
	 */
	private void findBlobs(Mat input) {
		...
		boolean found = false;
		Log.d("width", String.valueOf(input.height()));
		Log.d("x", "reset");
		for(x = x;x < input.width();x++){
			for(y = y;y < input.height();y++){
